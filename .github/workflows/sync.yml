...
추가 기능:
- 1.스탭 탭에서 TARGET_NAME이 월별로 몇 개 근무에 들어가 있는지 집계
- 텔레그램 보고서에 월별 근무 개수 포함
"""
...
# 월별 근무 개수 보고 범위
# False: 2026년 전체 근무 개수
# True: 오늘 이후 남은 근무 개수만 계산
STAFF_MONTHLY_REPORT_FUTURE_ONLY = False

# ===== 텔레그램 설정 =====
# 보안상 토큰은 코드에 직접 넣지 말고 GitHub Secrets에 넣는 것을 권장
    ...
    try:
        ...
        for row_idx, row in enumerate(values):
            for col_idx in range(2, min(len(row), 9)):
                ...
                if found_date:
                    start_part = found_slot.lower().split('-')[0]
                    start_hour = 8 if '8a' in start_part else 18
                    event_dt = found_date.replace(hour=start_hour, minute=0)

                    schedules.append({
                        'sheet': '1.스탭',
                        'name': f"근무 ({found_slot})",
                        'datetime': event_dt,
                        'slot': found_slot,
                        'location': '',
                        'details': (
...
def main():
    ...
    # 중요:
    # 월별 전체 근무 개수를 세려면, 아래 미래 일정 필터링 전에 계산해야 함
    ...
    for key in to_add:
        ...
        if s['sheet'] == '1.스탭':
            start_part = s.get('slot', '8a-6p').lower().split('-')[0]
            dur = 10 if '8a' in start_part else 14
